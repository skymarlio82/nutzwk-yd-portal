...
public class SysDictServiceImpl extends BaseServiceImpl<Sys_dict> implements SysDictService {
    ...
    public List<Sys_dict> getParentDicts() {
        return  this.query(Cnd.where("hasChildren", "=", 1).asc("location"));
    }
    ...
    public Map<String,Object> getJsonTree() {
        List<Sys_dict> dicts = this.getParentDicts();
        Map<String,Object> a_dicts  = new HashMap<String,Object>();
        List<Sys_dict> all_dicts = this.query();
        for (int i = 0; i < dicts.size(); i++) {
            Sys_dict dict = dicts.get(i);
            List<DictDto> sys_dicts = new ArrayList<DictDto>();
            for (int j = 0; j < all_dicts.size(); j++) {
                Sys_dict dt = all_dicts.get(j);
                if(dt.getParentId().equals(dict.getId())){
                    DictDto dto = new DictDto();
                    dto.setId(dt.getId());
                    dto.setName(dt.getName());
                    dto.setParentId(dt.getParentId());
                    sys_dicts.add(dto);
                }
            }
            a_dicts.put(dict.getCode(),sys_dicts);
        }
        return a_dicts;
    }
    ...
    public Map<String,String> getAllDataMap() {
        List<Sys_dict> dicts = this.getParentDicts();
        Map<String,String> a_dicts  = new HashMap<String,String>();
        List<Sys_dict> all_dicts = this.query();
        for (int i = 0; i < all_dicts.size(); i++) {
            Sys_dict dict = all_dicts.get(i);
            a_dicts.put(dict.getId(),dict.getName());
        }
        return a_dicts;
    }
