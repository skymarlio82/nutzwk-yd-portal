...
public class SysLoginController {
    ...
    public String login(HttpServletRequest req) {
        ...
        if (subject.isAuthenticated()) {
            return "redirect:/platform/cms/article";
        } else {
    ...
    public Object doLogin(@Attr("loginToken") AuthenticationToken token, HttpServletRequest req, HttpSession session) {
        ...
        String error = getReids().get("error");
...
//								.gimp(new FishEyeGimpyRenderer())
    ...
    /*@At("/judge")
    @Ok("json")
    public String judge(String judge) {
        String error = getReids().get("error");
        if (judge != null) {
            getReids().set("error", judge);
        }
        return error;
    }*/
    ...
    @Ok("json")
    @Filters
    public Object test(@Param("val") String val) {
        if(val == null||"".equals(val))
            return Result.success(getReids().get("testRedis"));
        getReids().set("testRedis", val);
